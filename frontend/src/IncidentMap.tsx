...
// OpenFreeMap supports keyless public use and supplies tile attribution and glyphs.
const style = 'https://tiles.openfreemap.org/styles/dark'

function incidentData(incidents: Incident[]): FeatureCollection<Point> {
  return {
    type: 'FeatureCollection',
    features: incidents.map(item => ({
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [item.location.longitude, item.location.latitude] },
      properties: { id: item.id, severity: item.severity, type: item.type, color: EVENT_VISUALS[item.type].color, symbol: EVENT_VISUALS[item.type].symbol },
    })),
  }
}
...
export function IncidentMap({ incidents, selectedId, region, onSelect, onPreview, onClusterPreview }: Props) {
  ...
  useEffect(() => {
    ...
    map.on('style.load', () => {
      ...
      map.addSource('incidents', { type: 'geojson', data: incidentData(incidentsRef.current), cluster: true, clusterMaxZoom: 6, clusterRadius: 48, clusterProperties })
      map.addLayer({ id: 'cluster-halo', type: 'circle', source: 'incidents', filter: ['has', 'point_count'], paint: { 'circle-color': 'rgba(91, 224, 218, 0.16)', 'circle-radius': ['interpolate', ['linear'], ['get', 'point_count'], 1, 25, 100, 34, 500, 43], 'circle-blur': 0.9 } })
      map.addLayer({ id: 'clusters', type: 'circle', source: 'incidents', filter: ['has', 'point_count'], paint: { 'circle-color': ['case', ['>', ['get', 'severe'], 0], '#d99a43', '#3b8d9b'], 'circle-radius': ['interpolate', ['linear'], ['get', 'point_count'], 1, 17, 100, 25, 500, 32], 'circle-stroke-color': '#9de8e3', 'circle-stroke-width': 1.5, 'circle-opacity': 0.94 } })
      map.addLayer({ id: 'cluster-count', type: 'symbol', source: 'incidents', filter: ['has', 'point_count'], layout: { 'text-field': ['get', 'point_count_abbreviated'], 'text-size': 12, 'text-font': ['Noto Sans Regular'] }, paint: { 'text-color': '#111a20' } })
      ...
      map.addLayer({ id: 'event-symbols', type: 'symbol', source: 'incidents', filter: ['!', ['has', 'point_count']], layout: { 'text-field': ['get', 'symbol'], 'text-font': ['Noto Sans Regular'], 'text-size': ['match', ['get', 'severity'], 'CRITICAL', 12, 'HIGH', 11, 9], 'text-allow-overlap': true }, paint: { 'text-color': '#10171d' } })
  ...
  useEffect(() => {
    // loaded() becomes false while tiles are in flight, but load fires only once.
    // Initialize from incidentsRef on style.load; update existing sources directly.
    const source = mapRef.current?.getSource('incidents') as GeoJSONSource | undefined
    source?.setData(incidentData(incidents))
  }, [incidents])
