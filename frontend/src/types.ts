...
export interface ProviderStatus { provider: string; status: 'LIVE' | 'DEGRADED' | 'SYNCING'; lastSuccessfulSync?: string; lastAttempt?: string; lastError?: string; dataAgeSeconds?: number | null; incidentCount: number }
export interface IncidentCollection { incidents: Incident[]; total: number; generatedAt: string; providers: ProviderStatus[] }
export interface IntelligenceStatus { available: boolean; configured: boolean; provider?: string; model?: string }
export interface AIIncidentBrief { headline: string; summary: string; keyPoints: string[]; sourcesUsed: string[]; generatedAt: string; cached: boolean }
