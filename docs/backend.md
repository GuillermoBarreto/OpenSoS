...
USGS defaults to 60-second refresh; EONET and GDACS to 15 minutes. Independent loops and per-provider locks let healthy providers refresh while another provider is slow, while serializing requests to the same provider (including bulk syncs). Successful sync timestamps record fetch completion. Failed syncs retain the last good snapshot and publish the truncated error.
