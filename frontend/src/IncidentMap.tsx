...
const style: maplibregl.StyleSpecification = { version: 8, sources: { osm: { type: 'raster', tiles: ['https://tile.openstreetmap.org/{z}/{x}/{y}.png'], tileSize: 256, attribution: '© OpenStreetMap contributors' } }, layers: [{ id: 'osm', type: 'raster', source: 'osm', paint: { 'raster-saturation': -0.82, 'raster-brightness-max': 0.63, 'raster-contrast': 0.3 } }] }
...
export function IncidentMap({ incidents, selectedId, region, onSelect, onPreview, onClusterPreview }: Props) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({ id: 'cluster-count', type: 'symbol', source: 'incidents', filter: ['has', 'point_count'], layout: { 'text-field': ['get', 'point_count_abbreviated'], 'text-size': 12, 'text-font': ['Open Sans Bold'] }, paint: { 'text-color': '#111a20' } })
